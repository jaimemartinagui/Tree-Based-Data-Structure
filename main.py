...
def generate_tree_based_data_structure(data: list[list]) -> dict:
    """Function that creates a tree-based data structure from a list of lists."""
    list_of_dicts = [_list_to_dict(d) for d in data]
    general_dict = {}
    for dict_ in list_of_dicts:
        general_dict = _join_two_dicts(general_dict, dict_)
    return general_dict


def _join_two_dicts(general_dict: dict, new_dict: dict) -> dict:
    ...
    if isinstance(general_dict, dict) and isinstance(new_dict, dict):
        if set(new_dict).issubset(set(general_dict)):
            key = list(new_dict)[0]
            new_dict = {key: _join_two_dicts(general_dict[key], new_dict[key])}
...
def _list_to_dict(list_: list) -> dict:
    ...
    if len(list_) == 0:
        ...
        dict_ = {list_[0]: _list_to_dict(list_[1:])}
    return dict_
